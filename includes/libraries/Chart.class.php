...
<section id="info" class="context-menu-'.$this->_diff.'-'.$this->_is_secondary.'-'.$this->_partner_dob.'-'.$this->_lang_code.' top-left-corner top-right-corner bottom-left-corner bottom-right-corner">
	...
	// Render controls
	function output_controls(): string {
...
function updateLanguageExplanation(langCode) {
	loadExplanationChartResults("'.$this->_dob.'","'.$this->_diff.'","1","'.date('Y-m-d',time()+86400*$this->_diff).'","'.$this->_partner_dob.'",langCode);
}
function changeLangExplanation(langCode) {
	manipulateLangEvent(langCode);
	updateLanguageExplanation(langCode);
}
$.contextMenu({
...
function updateLanguageMain(langCode) {
	loadResults("'.$this->_dob.'","'.$this->_diff.'","'.$this->_is_secondary.'","'.date('Y-m-d',time()+86400*$this->_diff).'","'.$this->_partner_dob.'",langCode);
}
function changeLangMain(langCode) {
	manipulateLangEvent(langCode);
	updateLanguageMain(langCode);
}
$.contextMenu({
	selector: ".context-menu-'.$this->_diff.'-'.$this->_is_secondary.'-'.$this->_partner_dob.'-'.$this->_lang_code.'",
	items: {
...
$("#stats").on("click", "#embed_box", function(){
	copyToClipboard("#embed_box");
	$("#embed_box").select();
}).on("click", "#go_to_chart", function(){
